...
Route::prefix('/dashboard')->middleware(['auth', 'role:admin'])->name('dashboard')->group(function () {
    ...
    Volt::route('/movies/search', 'dashboard.movies.movie-search')->name('.movies.search');
    ...
    Volt::route('/halls', 'dashboard.halls.halls-list')->name('.halls.list');

    Volt::route('/seats', 'dashboard.seats.seats-list')->name('.seats.list');
});
